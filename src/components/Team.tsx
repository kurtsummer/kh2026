      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24 space-y-6">
          ...
          <h2 className="text-5xl md:text-7xl font-black text-slate-900 leading-[0.9] tracking-tighter">
            ...
            <span className="text-sky-600 italic">unvergessliche Momente</span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-16 lg:gap-20">
          {team.map((member, index) => (
            <div key={index} className="group text-center">
              <div className="relative mb-12 rounded-[4rem] overflow-hidden aspect-[4/5] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] group-hover:shadow-[0_50px_100px_-20px_rgba(14,165,233,0.3)] transition-all duration-700 hover:-translate-y-4">
                <img 
                  src={member.image} 
                  alt={member.name} 
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-sky-900/90 via-sky-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-12">
              ...
              <h3 className="text-4xl font-black text-slate-900 mb-2 tracking-tighter italic">{member.name}</h3>
              <p className="text-sky-600 font-black uppercase tracking-[0.3em] text-[10px]">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
